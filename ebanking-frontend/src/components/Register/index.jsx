...
const Register = () => {
  ...
  const [registerUserQuery, registerUserQueryResponse] =
      ...
      <Form
        ...
        onFinish={() => {
          ...
          registerUserQuery({
            ...
            email: data?.email,
            accountType: Number(data?.actype),
            ...
            console.log({ res });
            if (res?.error) {
              if (res?.error?.data?.includes('An account already exists')) {
                message.error('Account already exists', 3);
                return;
              }
              message.error('User can not be registered', 3);
              return;
            }
            message.success(
              `User Registered successfully with ACNumber: ${res?.data?.accountNumberGenerated}`,
              3
            );
          });
        }}
