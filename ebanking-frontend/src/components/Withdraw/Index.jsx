...
import { useWithdrawMutation } from '../../store/features/apiSlice';
const { Option } = Select;
const Withdraw = () => {
  const dispatch = useDispatch();
  const [form] = Form.useForm();
  const navigate = useNavigate();
  const [withdrawQuery, withdrawQueryResponse] = useWithdrawMutation();
      ...
      <Form
        ...
        onFinish={() => {
          ...
          if (
            ...
            withdrawQuery(data).then((res) => {
              if (res?.error) {
                message.error('Invalid Pin', 3);
                return;
              }
            });
        ...
        initialValues={{ acnumber: validUser?.accountNumberGenerated, currency: 'GBP' }}
      >
        <Form.Item
          name='acnumber'
          label='From Account #'
