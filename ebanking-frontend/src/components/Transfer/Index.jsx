...
const Transfer = () => {
  ...
  const [transferQuery, transferQueryResponse] = useTransferMutation();
  const [verifyAccount, verifyAccountResponse] =
    useGetAccountByAccountNumberMutation();
      ...
      <Form
        ...
        onFinish={() => {
          ...
          if (
            ...
            console.log({ data });
            transferQuery(data).then((res) => {
              if (res?.error) {
                message.error('Transaction Unsuccessful', 3);
                return;
              }
        ...
        initialValues={{
          src_acnumber: validUser?.accountNumberGenerated,
          currency: 'GBP',
        }}
      >
        <Form.Item
        ...
        <div style={{ display: 'flex' }}>
          ...
          <Form.Item style={{ width: '20%' }}>
            <Button
              ...
              onClick={(e) => {
                // setIsVerified(true);
                verifyAccount(form.getFieldValue('dest_acnumber')).then(
                  (res) => {
                    if (res?.error) {
                    ...
                    setIsVerified(true);
                    dispatch(validateDest(res?.data));
                  }
                );
              }}
            >
            ...
            <Input
              ...
              onChange={(e) => {
                if (
                  Number(
                    Number(
                      validUser?.currentAccountBalance *
                        (selectedCurrency === 'GBP' ? 1 : 1.2)
